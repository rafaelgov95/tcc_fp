...
//        }
        for (int i = 0; i < inde_new_elo; ++i){
            ...
            elo_k1[elo_k1_current].size=inde_new_elo;
        inde_new_elo=0;
    }
...
// Algoritmo 2 Begin;
    ...
    if (threadIdx.x == elo_k1[elo_cur].size-1 ) {
        ...
        for (int i = 0; i < index_elo_put; ++i){
                    elo_x[i]= elo[i];
                    printf("Sem SetMap round :%d  valor %s\n",elo_cur,elo_x[i].ItemId);
        }
